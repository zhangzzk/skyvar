...
def make_uncorrelated_nz_maps(
    ...
    """Generate toy uncorrelated n(z) maps with per-pixel profile variations."""
    np.random.seed(seed)
    nz = len(z_edges) - 1
    ...
    # Per-pixel mean and width variations in linear space.
    ...
    for start in range(0, npix, chunk_size):
        ...
        # Lognormal PDF (numerically normalized on support).
        profiles = np.exp(
        ...
        # Local redshift standard deviation.
    ...
    # Subset of profiles for plotting.
...
def main() -> None:
    ...
    # Load catalog only to get a representative redshift distribution.
    ...
    if config.ANALYSIS_SETTINGS['smooth_nz']:
        ...
        # smooth_nz_preserve_moments expects shape (N, Nz).
    ...
    # For fair variance-mode comparison, use band-limited maps that match
    # the effective filtering in wtheta mode (anafast).
    ...
    for i in range(len(n_maps_partial)):
        # Reconstruct full map for SHT and zero-pad outside the mask.
    ...
    # Compare delta_w at the smallest theta.
    ...
    # Geometric factor from binned results (same logic as selection.py).
    ...
    # w_tot = Sum_{i,j} n_i n_j w_mat_ij
    ...
    # result.w_selection has shape (nz, nz, ntheta).
